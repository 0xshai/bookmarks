...
在`portable_config/mpv.conf` 中加入`border=no`这行代码，可以隐藏标题栏。
...
1. 下载 `yt-dlp.exe`，放到 mpv 根目录（与 `mpv.exe` 同级）,https://github.com/yt-dlp/yt-dlp/releases ,不要下别的，只下 yt-dlp.exe;
...
```
4. 一键更新 yt-dlp（以后新版本不用重新下）
在 mpv 同级目录打开 PowerShell，执行：
```ini
.\yt-dlp.exe -U
```
自动升级到最新版，解决网站接口失效、解析失败问题。
---
5. FFmpeg必装！Scoop 会自动把 ffmpeg.exe / ffprobe.exe 写入用户 PATH，安装完所有程序（mpv、yt-dlp、终端）都能全局调用，不用手动复制 exe 到 mpv 文件夹，也不用手动改系统 Path。
## 十、实用小技巧
